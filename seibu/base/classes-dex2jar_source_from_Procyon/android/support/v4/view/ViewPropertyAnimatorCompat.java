    ...
    static class BaseViewPropertyAnimatorCompatImpl implements ViewPropertyAnimatorCompatImpl
    {
        ...
        private void postStartMessage(final ViewPropertyAnimatorCompat viewPropertyAnimatorCompat, final View view) {
            ...
            if ((value = runnable) == null) {
                value = new Starter(viewPropertyAnimatorCompat, view);
                if (this.mStarterMap == null) {
                    this.mStarterMap = new WeakHashMap<View, Runnable>();
                }
                this.mStarterMap.put(view, value);
            }
            view.removeCallbacks(value);
            view.post(value);
        }
        
        private void removeStartMessage(final View key) {
            if (this.mStarterMap != null) {
                final Runnable runnable = this.mStarterMap.get(key);
                if (runnable != null) {
        ...
        void startAnimation(final ViewPropertyAnimatorCompat viewPropertyAnimatorCompat, final View key) {
            ...
            if (viewPropertyAnimatorListener != null) {
                viewPropertyAnimatorListener.onAnimationStart(key);
                viewPropertyAnimatorListener.onAnimationEnd(key);
            }
            if (mEndAction != null) {
                mEndAction.run();
            }
            if (this.mStarterMap != null) {
                this.mStarterMap.remove(key);
            }
        }
        ...
        class Starter implements Runnable
        {
            ...
            Starter(final ViewPropertyAnimatorCompat mVpa, final View referent) {
                this.mViewRef = new WeakReference<View>(referent);
                this.mVpa = mVpa;
            }
