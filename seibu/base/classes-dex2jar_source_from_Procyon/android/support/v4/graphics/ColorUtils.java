...
public final class ColorUtils
{
    ...
    public static int XYZToColor(@FloatRange(from = 0.0, to = 95.047) double n, @FloatRange(from = 0.0, to = 100.0) double n2, @FloatRange(from = 0.0, to = 108.883) double a) {
        ...
        if (a2 > 0.0031308) {
            n = 1.055 * Math.pow(a2, 0.4166666666666667) - 0.055;
        }
        else {
            n = a2 * 12.92;
        }
        if (a3 > 0.0031308) {
            n2 = 1.055 * Math.pow(a3, 0.4166666666666667) - 0.055;
        }
        else {
            n2 = a3 * 12.92;
        }
        ...
        return Color.rgb(constrain((int)Math.round(255.0 * n), 0, 255), constrain((int)Math.round(255.0 * n2), 0, 255), constrain((int)Math.round(255.0 * a), 0, 255));
    }
    ...
    public static int calculateMinimumAlpha(@ColorInt final int n, @ColorInt final int i, final float n2) {
        ...
        if (calculateContrast(setAlphaComponent(n, 255), i) < n2) {
            ...
            while (true) {
                ...
                if (calculateContrast(setAlphaComponent(n, n7), i) < n2) {
                    n5 = n7;
                }
                else {
                    n6 = n7;
    ...
    private static double pivotXyzComponent(final double a) {
        if (a > 0.008856) {
            return Math.pow(a, 0.3333333333333333);
        }
        return (903.3 * a + 16.0) / 116.0;
    }
