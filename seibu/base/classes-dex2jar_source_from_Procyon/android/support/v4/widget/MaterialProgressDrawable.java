...
class MaterialProgressDrawable extends Drawable implements Animatable
{
    ...
    private int evaluateColorChange(final float n, int i, int intValue) {
        final int intValue2 = i;
        i = (intValue2 >> 24 & 0xFF);
        final int n2 = intValue2 >> 16 & 0xFF;
        final int n3 = intValue2 >> 8 & 0xFF;
        final int n4 = intValue2 & 0xFF;
        intValue = intValue;
        return (int)(((intValue >> 24 & 0xFF) - i) * n) + i << 24 | (int)(((intValue >> 16 & 0xFF) - n2) * n) + n2 << 16 | (int)(((intValue >> 8 & 0xFF) - n3) * n) + n3 << 8 | (int)(((intValue & 0xFF) - n4) * n) + n4;
    }
    ...
    private static class Ring
    {
        ...
        private void drawTriangle(final Canvas canvas, final float n, final float n2, final Rect rect) {
            if (this.mShowArrow) {
                ...
                final float n3 = (float)((int)this.mStrokeInset / 2);
        ...
        public void setInsets(final int a, final int b) {
            final float n = (float)Math.min(a, b);
            float mStrokeInset;
            if (this.mRingCenterRadius <= 0.0 || n < 0.0f) {
                mStrokeInset = (float)Math.ceil(this.mStrokeWidth / 2.0f);
            }
            else {
                mStrokeInset = (float)(n / 2.0f - this.mRingCenterRadius);
            }
