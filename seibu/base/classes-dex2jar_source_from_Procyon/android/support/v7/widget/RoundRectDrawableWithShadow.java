...
class RoundRectDrawableWithShadow extends Drawable
{
    ...
    void setShadowSize(float n, float f) {
        if (n < 0.0f) {
            throw new IllegalArgumentException("Invalid shadow size " + n + ". Must be >= 0");
        }
        if (f < 0.0f) {
            throw new IllegalArgumentException("Invalid max shadow size " + f + ". Must be >= 0");
        }
        final float n2 = (float)this.toEven(n);
        final float mRawMaxShadowSize = (float)this.toEven(f);
        n = n2;
        if (n2 > mRawMaxShadowSize) {
            f = (n = mRawMaxShadowSize);
            if (!this.mPrintedShadowClipWarning) {
                this.mPrintedShadowClipWarning = true;
                n = f;
            }
        }
        if (this.mRawShadowSize == n && this.mRawMaxShadowSize == mRawMaxShadowSize) {
            return;
        }
        this.mRawShadowSize = n;
        this.mRawMaxShadowSize = mRawMaxShadowSize;
        this.mShadowSize = (float)(int)(1.5f * n + this.mInsetShadow + 0.5f);
