...
public class RecyclerView extends ViewGroup implements ScrollingView, NestedScrollingChild
{
    ...
    private void createLayoutManager(final Context context, String initargs, final AttributeSet set, final int i, final int j) {
        if (initargs != null) {
            initargs = ((String)initargs).trim();
            if (((String)initargs).length() != 0) {
                final String fullClassName = this.getFullClassName(context, (String)initargs);
                try {
                    Label_0114: {
                        if (!this.isInEditMode()) {
                            break Label_0114;
                        }
                        initargs = this.getClass().getClassLoader();
                        while (true) {
                            final Class<? extends LayoutManager> subclass = ((ClassLoader)initargs).loadClass(fullClassName).asSubclass(LayoutManager.class);
                            initargs = null;
                            try {
                                ...
                                initargs = new Object[] { context, set, i, j };
                                final Constructor<? extends LayoutManager> constructor2 = constructor;
                                constructor2.setAccessible(true);
                                this.setLayoutManager((LayoutManager)constructor2.newInstance((Object[])initargs));
                                return;
                                initargs = context.getClassLoader();
                            }
                    ...
                    throw new IllegalStateException(set.getPositionDescription() + ": Unable to find LayoutManager " + fullClassName, cause3);
                }
                ...
                catch (InstantiationException cause5) {
                    throw new IllegalStateException(set.getPositionDescription() + ": Could not instantiate the LayoutManager: " + fullClassName, cause5);
                }
                catch (IllegalAccessException cause6) {
                    throw new IllegalStateException(set.getPositionDescription() + ": Cannot access non-public constructor " + fullClassName, cause6);
                }
                catch (ClassCastException cause7) {
                    throw new IllegalStateException(set.getPositionDescription() + ": Class is not a LayoutManager " + fullClassName, cause7);
                }
            }
        }
    }
    ...
    public boolean fling(int max, int max2) {
        if (this.mLayout == null) {
            ...
            if ((a != 0 || max != 0) && !this.dispatchNestedPreFling((float)a, (float)max)) {
                final boolean b = canScrollHorizontally || canScrollVertically;
                this.dispatchNestedFling((float)a, (float)max, b);
                if (this.mOnFlingListener != null && this.mOnFlingListener.onFling(a, max)) {
                    return true;
                }
                if (b) {
        ...
        while (true) {
            switch (actionMasked) {
                ...
                case 2: {
                    ...
                    if (this.scrollByInternal(n22, n23, obtain)) {
                        this.getParent().requestDisallowInterceptTouchEvent(true);
                    }
                    n2 = n;
                    if (this.mGapWorker != null) {
                        ...
                        this.mGapWorker.postFromTraversal(this, n15, n16);
                        n2 = n;
                    }
                    continue;
                }
        ...
        public static int chooseSize(int n, final int n2, final int n3) {
            ...
            switch (mode) {
                ...
                case Integer.MIN_VALUE: {
                    return Math.min(a, Math.max(n2, n3));
                }
            }
        }
    ...
    public final class Recycler
    {
        ...
        public void bindViewToPosition(final View view, final int i) {
            ...
            if (positionOffset < 0 || positionOffset >= RecyclerView.this.mAdapter.getItemCount()) {
                throw new IndexOutOfBoundsException("Inconsistency detected. Invalid item position " + i + "(offset:" + positionOffset + ")." + "state:" + RecyclerView.this.mState.getItemCount());
            }
                        ...
                        if (RecyclerView.ALLOW_THREAD_GAP_WORK) {
                            ...
                            if (size > 0) {
                                ...
                                if (!RecyclerView.this.mPrefetchRegistry.lastPrefetchIncludedPosition(viewHolder.mPosition)) {
                                    int index2;
                                    for (index2 = size - 1; index2 >= 0 && RecyclerView.this.mPrefetchRegistry.lastPrefetchIncludedPosition(this.mCachedViews.get(index2).mPosition); --index2) {}
                                    index = index2 + 1;
                                }
                            }
                        }
        ...
        ViewHolder tryGetViewHolderForPositionByDeadline(final int n, final boolean b, final long n2) {
            ...
            boolean b3 = b2;
            if (changedScrapViewForPosition == null) {
                final ViewHolder viewHolder = scrapOrHiddenOrCachedHolderForPosition = this.getScrapOrHiddenOrCachedHolderForPosition(n, b);
                b3 = b2;
                if (viewHolder != null) {
            ...
            if (RecyclerView.this.mState.isPreLayout() && viewHolder2.isBound()) {
                ...
                tryBindViewHolderByDeadline = this.tryBindViewHolderByDeadline(viewHolder2, RecyclerView.this.mAdapterHelper.findPositionOffset(n), n, n2);
            }
