...
class TransitionManagerPort
{
    ...
    static ArrayMap<ViewGroup, ArrayList<TransitionPort>> getRunningTransitions() {
        ...
        if (weakReference != null) {
            ...
            if (weakReference.get() != null) {
                return (ArrayMap<ViewGroup, ArrayList<TransitionPort>>)value.get();
            }
        }
        final Reference<T> value = (Reference<T>)new WeakReference<Object>(new ArrayMap<ViewGroup, ArrayList<TransitionPort>>());
        TransitionManagerPort.sRunningTransitions.set((WeakReference<ArrayMap<ViewGroup, ArrayList<TransitionPort>>>)value);
        return (ArrayMap<ViewGroup, ArrayList<TransitionPort>>)value.get();
    }
    ...
    private static class MultiListener implements ViewTreeObserver$OnPreDrawListener, View$OnAttachStateChangeListener
    {
        ...
        public boolean onPreDraw() {
            ...
            final ArrayList<TransitionPort> c = runningTransitions.get(this.mSceneRoot);
            ArrayList<TransitionPort> list = null;
            ArrayList<TransitionPort> list2;
            if (c == null) {
                list2 = new ArrayList<TransitionPort>();
                runningTransitions.put(this.mSceneRoot, list2);
            }
            else {
                list2 = c;
                if (c.size() > 0) {
                    list = new ArrayList<TransitionPort>(c);
                    list2 = c;
                }
            }
            ...
            this.mTransition.addListener((TransitionPort.TransitionListener)new TransitionPort.TransitionListenerAdapter() {
                @Override
                public void onTransitionEnd(final TransitionPort o) {
