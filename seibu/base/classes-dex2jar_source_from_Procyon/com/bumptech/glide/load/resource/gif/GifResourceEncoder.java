...
public class GifResourceEncoder implements ResourceEncoder<GifDrawable>
{
    ...
    private boolean writeDataDirect(final byte[] b, final OutputStream outputStream) {
        try {
    ...
    public boolean encode(final Resource<GifDrawable> resource, OutputStream transformedFrame) {
        ...
        if (frameTransformation instanceof UnitTransformation) {
            b = this.writeDataDirect(gifDrawable.getData(), transformedFrame);
        }
        else {
            ...
            b = buildEncoder.finish();
            if (Log.isLoggable("GifEncoder", 2)) {
                Log.v("GifEncoder", "Encoded gif with " + decodeHeaders.getFrameCount() + " frames and " + gifDrawable.getData().length + " bytes in " + LogTime.getElapsedMillis(logTime) + " ms");
                return b;
            }
        }
        return b;
