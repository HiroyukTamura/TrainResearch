...
{
    private static final SimpleArrayMap<String, String> zzaHo;
    
    static {
        zzaHo = new SimpleArrayMap<String, String>();
    }
    
    private static String zzaB(Context string) {
        final String packageName = ((Context)string).getPackageName();
        try {
            string = zzbha.zzaP((Context)string).zzcM(packageName).toString();
    ...
    @Nullable
    public static String zzg(final Context context, final int i) {
    ...
    public static String zzh(final Context context, final int n) {
        String s;
        if (n == 6) {
            s = zzz(context, "common_google_play_services_resolution_required_title");
        }
        else {
            s = zzg(context, n);
        }
    ...
    public static String zzi(final Context context, final int n) {
        final Resources resources = context.getResources();
        final String zzaB = zzaB(context);
        switch (n) {
            default: {
    ...
    public static String zzj(final Context context, final int n) {
        if (n == 6) {
            return zzl(context, "common_google_play_services_resolution_required_text", zzaB(context));
        }
        return zzi(context, n);
    }
    ...
    private static String zzl(final Context context, String s, final String s2) {
        final Resources resources = context.getResources();
        String format;
        s = (format = zzz(context, s));
        if (s == null) {
            format = resources.getString(R.string.common_google_play_services_unknown_issue);
        }
        return String.format(resources.getConfiguration().locale, format, s2);
    }
    
    @Nullable
    private static String zzz(final Context context, final String s) {
        final int identifier;
        synchronized (zzs.zzaHo) {
            final String s2 = zzs.zzaHo.get(s);
            if (s2 != null) {
                return s2;
            }
