...
public final class Version
{
    ...
    private Version(int i, final int[] alignmentPatternCenters, final ECBlocks... ecBlocks) {
        final int n = 0;
        this.versionNumber = i;
        this.alignmentPatternCenters = alignmentPatternCenters;
        this.ecBlocks = ecBlocks;
        final int n2 = 0;
        final int ecCodewordsPerBlock = ecBlocks[0].getECCodewordsPerBlock();
        final ECB[] ecBlocks2 = ecBlocks[0].getECBlocks();
        final int length = ecBlocks2.length;
        i = n;
        int totalCodewords = n2;
        while (i < length) {
            final ECB ecb = ecBlocks2[i];
            totalCodewords += ecb.getCount() * (ecb.getDataCodewords() + ecCodewordsPerBlock);
            ++i;
        }
    ...
    static Version decodeVersionInformation(final int n) {
        int n2 = Integer.MAX_VALUE;
        int n3 = 0;
        int n5;
        for (int i = 0; i < Version.VERSION_DECODE_INFO.length; ++i, n2 = n5) {
            final int n4 = Version.VERSION_DECODE_INFO[i];
            if (n4 == n) {
                return getVersionForNumber(i + 7);
            }
            final int numBitsDiffering = FormatInformation.numBitsDiffering(n, n4);
            if (numBitsDiffering < (n5 = n2)) {
                n3 = i + 7;
                n5 = numBitsDiffering;
            }
        }
        if (n2 <= 3) {
            return getVersionForNumber(n3);
        }
        return null;
    }
    
    public static Version getProvisionalVersionForDimension(final int n) throws FormatException {
        if (n % 4 != 1) {
            throw FormatException.getFormatInstance();
        }
        try {
            return getVersionForNumber((n - 17) / 4);
        }
    ...
    public static Version getVersionForNumber(final int n) {
        if (n <= 0 || n > 40) {
            throw new IllegalArgumentException();
        }
        return Version.VERSIONS[n - 1];
    }
    
    BitMatrix buildFunctionPattern() {
        final int dimensionForVersion = this.getDimensionForVersion();
        final BitMatrix bitMatrix = new BitMatrix(dimensionForVersion);
        bitMatrix.setRegion(0, 0, 9, 9);
        bitMatrix.setRegion(dimensionForVersion - 8, 0, 8, 9);
        bitMatrix.setRegion(0, dimensionForVersion - 8, 9, 8);
        for (int length = this.alignmentPatternCenters.length, i = 0; i < length; ++i) {
            final int n = this.alignmentPatternCenters[i];
            for (int j = 0; j < length; ++j) {
                if ((i != 0 || (j != 0 && j != length - 1)) && (i != length - 1 || j != 0)) {
                    bitMatrix.setRegion(this.alignmentPatternCenters[j] - 2, n - 2, 5, 5);
                }
            }
        }
    ...
    public ECBlocks getECBlocksForLevel(final ErrorCorrectionLevel errorCorrectionLevel) {
    ...
    public static final class ECBlocks
    {
        ...
        public int getNumBlocks() {
            int n = 0;
            final ECB[] ecBlocks = this.ecBlocks;
