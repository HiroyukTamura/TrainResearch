...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        if (min > 0) {
            this.f46mX = new float[min];
            this.f47mY = new float[min];
            float[] fArr = new float[2];
            for (int i2 = 0; i2 < min; i2++) {
                pathMeasure.getPosTan((((float) i2) * length) / ((float) (min - 1)), fArr, (float[]) null);
                this.f46mX[i2] = fArr[0];
                this.f47mY[i2] = fArr[1];
            }
            if (((double) Math.abs(this.f46mX[0])) <= 1.0E-5d && ((double) Math.abs(this.f47mY[0])) <= 1.0E-5d) {
                int i3 = min - 1;
                if (((double) Math.abs(this.f46mX[i3] - 1.0f)) <= 1.0E-5d && ((double) Math.abs(this.f47mY[i3] - 1.0f)) <= 1.0E-5d) {
                    float f = 0.0f;
                    int i4 = 0;
                    while (i < min) {
                        float[] fArr2 = this.f46mX;
                        int i5 = i4 + 1;
                        float f2 = fArr2[i4];
                        if (f2 >= f) {
                            fArr2[i] = f2;
            ...
            StringBuilder a = C0681a.m330a("The Path must start at (0,0) and end at (1,1) start: ");
            a.append(this.f46mX[0]);
            a.append(",");
            a.append(this.f47mY[0]);
            a.append(" end:");
            int i6 = min - 1;
            a.append(this.f46mX[i6]);
            a.append(",");
            a.append(this.f47mY[i6]);
    ...
    private void parseInterpolatorFromTypeArray(TypedArray typedArray, XmlPullParser xmlPullParser) {
        if (TypedArrayUtils.hasAttribute(xmlPullParser, "pathData")) {
            ...
            throw new InflateException(C0681a.m320a("The path is null, which is created from ", namedString));
    ...
    public float getInterpolation(float f) {
        ...
        int i = 0;
        int length = this.f46mX.length - 1;
        while (length - i > 1) {
            int i2 = (i + length) / 2;
            if (f < this.f46mX[i2]) {
                length = i2;
