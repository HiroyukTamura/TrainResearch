...
class SessionReportingCoordinator implements CrashlyticsLifecycleEvents {
    ...
    public boolean onReportSendComplete(@NonNull Task<CrashlyticsReportWithSessionId> task) {
        if (task.isSuccessful()) {
            ...
            StringBuilder a = C0681a.m330a("Crashlytics report successfully enqueued to DataTransport: ");
            a.append(result.getSessionId());
            logger.mo25712d(a.toString());
            this.reportPersistence.deleteFinalizedReport(result.getSessionId());
            return true;
        }
        Logger.getLogger().mo25713d("Crashlytics report could not be enqueued to DataTransport", task.getException());
        return false;
    }

    private void persistEvent(@NonNull Throwable th, @NonNull Thread thread, @NonNull String str, long j, boolean z) {
        String str2 = this.currentSessionId;
        if (str2 == null) {
            Logger.getLogger().mo25712d("Cannot persist event, no currently open session");
            return;
        }
    ...
    public void persistFatalEvent(@NonNull Throwable th, @NonNull Thread thread, long j) {
        persistEvent(th, thread, "crash", j, true);
    }

    public void persistNonFatalEvent(@NonNull Throwable th, @NonNull Thread thread, long j) {
    ...
    public void persistUserId() {
        ...
        if (str == null) {
            Logger.getLogger().mo25712d("Could not persist user ID; no current session");
            return;
        }
        String userId = this.reportMetadata.getUserId();
        if (userId == null) {
            Logger.getLogger().mo25712d("Could not persist user ID; no user ID available");
        } else {
    ...
    public Task<Void> sendReports(@NonNull Executor executor, @NonNull DataTransportState dataTransportState) {
        if (dataTransportState == DataTransportState.NONE) {
            Logger.getLogger().mo25712d("Send via DataTransport disabled. Removing DataTransport reports.");
            this.reportPersistence.deleteAllReports();
        ...
        for (CrashlyticsReportWithSessionId next : loadFinalizedReports) {
            if (next.getReport().getType() != CrashlyticsReport.Type.NATIVE || dataTransportState == DataTransportState.ALL) {
                ...
                Logger.getLogger().mo25712d("Send native reports via DataTransport disabled. Removing DataTransport reports.");
