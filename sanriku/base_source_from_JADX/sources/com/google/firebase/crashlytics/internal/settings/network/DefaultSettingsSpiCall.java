...
public class DefaultSettingsSpiCall extends AbstractSpiCall implements SettingsSpiCall {
    ...
    private JSONObject getJsonObjectFrom(String str) {
        try {
            ...
            StringBuilder a = C0681a.m330a("Failed to parse settings JSON from ");
            a.append(getUrl());
            logger2.mo25713d(a.toString(), e);
            Logger logger3 = this.logger;
            logger3.mo25712d("Settings response " + str);
            return null;
        }
    }
    ...
    public JSONObject handleResponse(HttpResponse httpResponse) {
        ...
        Logger logger2 = this.logger;
        logger2.mo25712d("Settings result was: " + code);
        if (requestWasSuccessful(code)) {
            return getJsonObjectFrom(httpResponse.body());
        }
        Logger logger3 = this.logger;
        StringBuilder a = C0681a.m330a("Failed to retrieve settings from ");
        a.append(getUrl());
        logger3.mo25714e(a.toString());
        return null;
    }
    ...
    public JSONObject invoke(SettingsRequest settingsRequest, boolean z) {
        if (z) {
            try {
                ...
                Logger logger2 = this.logger;
                logger2.mo25712d("Requesting settings from " + getUrl());
                Logger logger3 = this.logger;
                logger3.mo25712d("Settings query params were: " + queryParamsFor);
                HttpResponse execute = applyHeadersTo.execute();
                Logger logger4 = this.logger;
                logger4.mo25712d("Settings request ID: " + execute.header(AbstractSpiCall.HEADER_REQUEST_ID));
                return handleResponse(execute);
            } catch (IOException e) {
                this.logger.mo25715e("Settings request failed.", e);
                return null;
            }
        } else {
