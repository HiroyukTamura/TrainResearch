...
public class CrashlyticsCore {
    ...
    public Task<Void> doBackgroundInitialization(SettingsDataProvider settingsDataProvider) {
        ...
        try {
            ...
            if (!settings.getFeaturesData().collectReports) {
                Logger.getLogger().mo25712d("Collection of crash reports disabled in Crashlytics settings.");
                return Tasks.forException(new RuntimeException("Collection of crash reports disabled in Crashlytics settings."));
            }
            if (!this.controller.finalizeSessions(settings.getSessionData().maxCustomExceptionEvents)) {
                Logger.getLogger().mo25712d("Could not finalize previous sessions.");
            }
            ...
            Logger.getLogger().mo25715e("Crashlytics encountered a problem during asynchronous initialization.", e);
    ...
    private void finishInitSynchronously(final SettingsDataProvider settingsDataProvider) {
        ...
        Logger.getLogger().mo25712d("Crashlytics detected incomplete initialization on previous app launch. Will initialize synchronously.");
        try {
    ...
    static boolean isBuildIdValid(String str, boolean z) {
        if (!z) {
            Logger.getLogger().mo25712d("Configured not to require a build ID.");
            return true;
        ...
        this.backgroundWorker.submit(new Callable<Boolean>() {
            public Boolean call() throws Exception {
                try {
                    ...
                    logger.mo25712d("Initialization marker file removed: " + remove);
                    return Boolean.valueOf(remove);
                } catch (Exception e) {
                    Logger.getLogger().mo25715e("Problem encountered deleting Crashlytics initialization marker.", e);
                    return false;
                }
            }
        });
    ...
    public void markInitializationStarted() {
        ...
        Logger.getLogger().mo25712d("Initialization marker file created.");
    }
    ...
    public boolean onPreExecute(SettingsDataProvider settingsDataProvider) {
        ...
        Logger logger = Logger.getLogger();
        logger.mo25712d("Mapping file ID is: " + mappingFileId);
        if (isBuildIdValid(mappingFileId, CommonUtils.getBooleanResourceValue(this.context, CRASHLYTICS_REQUIRE_BUILD_ID, true))) {
            ...
            try {
                ...
                logger2.mo25716i("Initializing Crashlytics " + getVersion());
                ...
                logger3.mo25712d("Installer package name is: " + create.installerPackageName);
                ...
                if (!didPreviousInitializationFail || !CommonUtils.canTryConnection(this.context)) {
                    Logger.getLogger().mo25712d("Exception handling initialization successful");
                    return true;
                }
                Logger.getLogger().mo25712d("Crashlytics did not finish previous background initialization. Initializing synchronously.");
                finishInitSynchronously(settingsDataProvider);
                return false;
            } catch (Exception e) {
                Logger.getLogger().mo25715e("Crashlytics was not started due to an exception during initialization", e);
