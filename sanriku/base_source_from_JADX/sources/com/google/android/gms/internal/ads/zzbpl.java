package com.google.android.gms.internal.ads;

public final class zzbpl implements zzegz<zzbnv> {
    private final zzehm<zzbrr> zzewi;
    private final zzehm<zzbsh> zzexi;
    private final zzehm<zzdha> zzfgu;
    private final zzehm<zzdgo> zzfkc;
    private final zzehm<zzder> zzfma;
    private final zzehm<zzbqs> zzfmb;

    private zzbpl(zzehm<zzdha> zzehm, zzehm<zzdgo> zzehm2, zzehm<zzbrr> zzehm3, zzehm<zzbsh> zzehm4, zzehm<zzder> zzehm5, zzehm<zzbqs> zzehm6) {
        this.zzfgu = zzehm;
        this.zzfkc = zzehm2;
        this.zzewi = zzehm3;
        this.zzexi = zzehm4;
        this.zzfma = zzehm5;
        this.zzfmb = zzehm6;
    }

    public static zzbpl zzb(zzehm<zzdha> zzehm, zzehm<zzdgo> zzehm2, zzehm<zzbrr> zzehm3, zzehm<zzbsh> zzehm4, zzehm<zzder> zzehm5, zzehm<zzbqs> zzehm6) {
        return new zzbpl(zzehm, zzehm2, zzehm3, zzehm4, zzehm5, zzehm6);
    }

    public final /* synthetic */ Object get() {
        return new zzbnv(this.zzfgu.get(), this.zzfkc.get(), this.zzewi.get(), this.zzexi.get(), this.zzfma.get(), this.zzfmb.get());
    }
}
